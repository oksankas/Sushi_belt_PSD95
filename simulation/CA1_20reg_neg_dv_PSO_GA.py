...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('CA1_20reg_neg_dv_3m_cf.csv',header=False,mode='a')
...
plt.savefig('CA1_20reg_neg_dv_3m_PSO.png')
plt.savefig('CA1_20reg_neg_dv_3m_PSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('CA1_20reg_neg_dv_3m_best_pso.csv')
...
for cnt in range(200):
    ...
    df = pd.DataFrame(best_line).T
    log.info(f'best df={df}')
    df.to_csv('CA1_20reg_neg_dv_3m_best_pso_ga.csv',header=False,mode='a')
    Y_history = pd.DataFrame(ga.all_history_Y)
    fig, ax = plt.subplots(2, 1)
    ax[0].plot(Y_history.index, Y_history.values, '.', color='red')
    Y_history.min(axis=1).cummin().plot(kind='line')
    #plt.show()
    plt.savefig('CA1_20reg_neg_dv_3m_GA.png')
    plt.savefig('CA1_20reg_neg_dv_3m_GA.pdf')
    chrom = ga.Chrom
