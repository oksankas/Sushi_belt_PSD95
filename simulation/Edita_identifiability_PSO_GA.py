...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('Edita_cfi.csv',header=False,mode='a')
...
def profileChiSq(pn,pv=0,pso_iter=15,nm_iter=100,ga_cycles=200):
    ...
    def costFunctionC(cpar):
        ...
        pDF['Cost'] = cost
        pDF.to_csv('Edita_cf_idnt.csv',header=False,mode='a')
        return cost
    ...
    #plt.savefig('EditaPSOident.png')
    #plt.savefig('EditaPSOident.pdf')
    ...
    # psoDF['ParamVal']=pval
    # psoDF['Cost']=psoy
    # psoDF.to_csv('Edita_best_pso.csv')
    ...
    for cnt in range(ga_cycles):
        ...
        # df.to_csv('Edita_ident_pso_ga.csv',header=False,mode='a')
...
for pni in parnames :
    for pvj in parvals :
        ...
        bdf.to_csv('Edita_ident.csv', header=False, mode='a')
        chiCounter += 1
