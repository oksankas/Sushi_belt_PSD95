...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('CA1_cfi.csv',header=False,mode='a')
...
upbga=np.array([1, -1, -1, 1-1e-7, 1, 1, 1])
...
def profileChiSq(pn,pv=0,pso_iter=15,nm_iter=100,ga_cycles=200):
    ...
    def costFunctionC(cpar):
        ...
        pDF['Cost'] = cost
        pDF.to_csv('CA1_cf_idnt.csv',header=False,mode='a')
        return cost

    #log.info(f'{pn}={pval}, Run Nelder-Mead from the centre')
    #result = minimize(costFunctionC, (clowb+0.5*(cupbga-clowb)), method='nelder-mead',bounds=cbnds,options={'maxfev':10})
    #log.info(f'{pn}={pval}, best_x is {result.x}, best_y is {result.fun}')
    log.info(f'{pn}={pval}, Start PSO')
    # Population size for the PSO should be at least 2*numPar and in ideal situation 5*numPar
    #pso = PSO(func=costFunction, n_dim=7, pop=40, max_iter=15, lb=lowb,
    ...
    #plt.savefig('CA1_PSOident.png')
    #plt.savefig('CA1_PSOident.pdf')


    log.info(f'{pn}={pval}, Run Nelder-Mead on PSO result')
    #result = minimize(costFunctionC, pso.gbest_x, method='nelder-mead',bounds=bnds,options={'maxiter':100})
    ...
    #psox=prepPar(pso.gbest_x,pn,pval,pidx,bidx)
    psox = pso.pbest_x
    psoy=pso.pbest_y
    ...
    # initDF=pd.read_csv('CA1_best.csv')
    ...
    for cnt in range(ga_cycles):
        ...
        # df['ParamVal'] = pval
        # df['Cost'] = best_y
        # log.info(f'{pn}={pval}, best df={df}')
        # df.to_csv('CA1_ident_pso_ga.csv',header=False,mode='a')
        # Y_history = pd.DataFrame(ga.all_history_Y)
        #fig, ax = plt.subplots(2, 1)
        #ax[0].plot(Y_history.index, Y_history.values, '.', color='red')
        #Y_history.min(axis=1).cummin().plot(kind='line')
        #plt.show()
        #plt.savefig('CA1_GArest.png')
        #plt.savefig('CA1_GArest.pdf')
        chrom = ga.Chrom
        ...
        if bpar_dist > 1e-7 :
            ...
            #result = minimize(costFunctionC, ga.best_x, method='nelder-mead', bounds=bnds, options={'maxiter': 100})
...
for pni in parnames :
    for pvj in parvals :
        ...
        bdf.to_csv('CA1_ident.csv', header=False, mode='a')
        chiCounter += 1
