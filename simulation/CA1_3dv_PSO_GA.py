...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('CA1_3dv_cf.csv',header=False,mode='a')
...
log.info('Start PSO')
pso = PSO(func=costFunction, n_dim=9, pop=40, max_iter=15, lb=lowb,
        ub=upbga, w=0.8, c1=0.5, c2=0.5)
pso.run()
log.info(f'best_x is {pso.gbest_x}, best_y is {pso.gbest_y}')
plt.plot(pso.gbest_y_hist)
plt.savefig('CA1_3dv_PSO.png')
plt.savefig('CA1_3dv_PSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('CA1_3dv_best_pso.csv')
idx=np.argsort(psoy.flatten())
gainit=np.vstack([psox[idx[:17]],result.x])
# log.info('Read GA population')
# initDF=pd.read_csv('CA1_best.csv')
log.info('GA starts')
ga = RCGA(func=costFunction, n_dim=9, size_pop=18, max_iter=50000, prob_mut=0.01, lb=lowb,
        ub=upbga)
...
for cnt in range(200):
    ...
    df = pd.DataFrame(best_line).T
    log.info(f'best df={df}')
    df.to_csv('CA1_3dv_best_pso_ga.csv',header=False,mode='a')
    Y_history = pd.DataFrame(ga.all_history_Y)
    fig, ax = plt.subplots(2, 1)
    ax[0].plot(Y_history.index, Y_history.values, '.', color='red')
    Y_history.min(axis=1).cummin().plot(kind='line')
    #plt.show()
    plt.savefig('CA1_3dv_GA.png')
    plt.savefig('CA1_3dv_GA.pdf')
    chrom = ga.Chrom
