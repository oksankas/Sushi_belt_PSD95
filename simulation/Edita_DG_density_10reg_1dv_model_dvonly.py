...
from PyNeuronToolbox.morphology import dist_between,allsec_preorder,load
...
opt_par =  [0.010254232013526715,-5.638032179385583,0.4901989246102767,1.0346217672476097e-07,0.029242261750488406,0.01218622680694604,0.041390587450443085,0.06704294550203627,0.08976537459084287,0.1513671679744187,0.6105826545927185,0.543743857008341,0.28125287183335723,-5.336672864778542]
...
def calcUtrace(par,delta=bgSignal):
    ...
    dv = (10 ** par[0])*np.ones(N)
    for k in range(N):
        if itarg[k] > 2:
            utarg[k] = opt_par[itarg[k]]
    utarg /= np.sum(utarg)
    K = np.sum(utarg) / N
    x = trafficking_solution(F * utarg + (1 - F) * K)
    a = (1 / (1 + x))
