...
def calcUtrace(par,delta=bgSignal,time=day7,nframes=10):
    F = par0[0]
    Ctau = 10 ** par0[1]
    mProp = par0[2]
    dvA = par0[3]
    dvB = par0[4]
    dv = np.zeros(N)
    tr = (10 ** par[1]) *np.ones(N)
    tr[0] = 10 ** par[0]
    reatt = 10 ** par[2]
    utarg = delta*np.ones(N)
    for k in range(N):
        if itarg[k] > 4:
            utarg[k] = par0[itarg[k]]
    ...
    u0 = np.zeros(2*N+1)
    u0[2*N]=1
    utrace = sim_time(A, u0, time=time,nframes=nframes)
    return utrace
...
logFolder = '.'
modelPrefix = 'Edita_20reg_1dv_3M_somaplus_reattach'
parnames=['trSoma','trNeuro','reattachmentSoma']
lowb=np.array([-7,-7,-7])
upbga=np.array([3,3,1])
