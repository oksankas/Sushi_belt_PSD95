...
log.info('GA starts')
ga = GA(func=costFunction, n_dim=7, size_pop=14, max_iter=50000, prob_mut=0.01, lb=[0, -8, -8, 1e-3, 1e-3, 1e-3, 1e-3],
        ub=[1, -1, -1, 1, 1, 1, 1], precision=[1e-7, 1e-1, 1e-1, 1e-3, 1e-7, 1e-7, 1e-7])
for cnt in range(200):
    ...
    df.to_csv('Edita_best.csv',header=False,mode='a')
    Y_history = pd.DataFrame(ga.all_history_Y)
    fig, ax = plt.subplots(2, 1)
    ax[0].plot(Y_history.index, Y_history.values, '.', color='red')
    Y_history.min(axis=1).cummin().plot(kind='line')
    #plt.show()
    plt.savefig('EditaGA.png')
    plt.savefig('EditaGA.pdf')
