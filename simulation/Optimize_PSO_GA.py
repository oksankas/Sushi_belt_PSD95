...
def optimizeCF(model,pso_iter=15,nm_iter=100,ga_cycles=200, prob_mut=0.01, prob_cros=0.9):
    ...
    df.to_csv(f'{logFolder}/{model.modelPrefix}_{runPrefix}_best_pso_ga.csv',header=True,mode='w')
    ...
    df.index = [model.cfCounter]
    df.to_csv(f'{logFolder}/{model.modelPrefix}_{runPrefix}_best_pso_ga.csv',header=False,mode='a')
