...
log = logging.getLogger("DG_GA-logger")
...
def calcUtrace(par,delta=bgSignal,time=day7):
    F = par[0]
    Ctau = 10 ** par[1]
    mProp = par[2]
    utarg = delta*np.ones(N)
    dv = (10 ** par[13])*np.ones(N)
    for k in range(N):
        if itarg[k] > 2:
            utarg[k] = par[itarg[k]]
    utarg /= np.sum(utarg)
