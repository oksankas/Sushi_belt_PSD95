...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('Edita_20reg_1dv_18m_cf.csv',header=False,mode='a')
...
plt.savefig('Edita_20reg_1dv_18m_PSO.png')
plt.savefig('Edita_20reg_1dv_18m_PSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('Edita_20reg_1dv_18m_best_pso.csv')
...
for cnt in range(200):
    ...
    #plt.show()
    plt.savefig('Edita_20reg_1dv_18m_GA.png')
    plt.savefig('Edita_20reg_1dv_18m_GA.pdf')
    chrom = ga.Chrom
    ...
    if bpar_dist > 1e-7:
        ...
        df.index = [cfCounter]
        #log.info(f'best df={df}')
        df.to_csv('Edita_20reg_1dv_18m_best_pso_ga.csv', header=False, mode='a')
    else :
        bestX = ga.best_x
        bestY = best_y
