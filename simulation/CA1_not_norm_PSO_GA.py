...
for i in range(N):
    if d7w[i]<0:
        d7w[i] = bgSignal
target = np.array(expD[f"{cname}_MEAN"])/np.sum(expD["D0W3_MEAN"]) #norm target to Day0 sum to take into accound degradation
...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('CA1_nn_cf.csv',header=False,mode='a')
...
plt.savefig('CA1_PSO.png')
plt.savefig('CA1_PSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('CA1_nn_best_pso.csv')
idx=np.argsort(psoy.flatten())
gainit=np.vstack([psox[idx[:13]],result.x])
# log.info('Read GA population')
# initDF=pd.read_csv('CA1_best.csv')
log.info('GA starts')
ga = RCGA(func=costFunction, n_dim=7, size_pop=14, max_iter=50000, prob_mut=0.01, lb=lowb,
        ub=upbga)
...
for cnt in range(200):
    ...
    df = pd.DataFrame(best_line).T
    log.info(f'best df={df}')
    df.to_csv('CA1_nn_best_pso_ga.csv',header=False,mode='a')
