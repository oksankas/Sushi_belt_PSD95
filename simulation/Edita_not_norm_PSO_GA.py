...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('Edita_nn_cf.csv',header=False,mode='a')
...
plt.savefig('EditaPSO.png')
plt.savefig('EditaPSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('Edita_nn_best_pso.csv')
...
for cnt in range(200):
    ...
    df = pd.DataFrame(best_line).T
    log.info(f'best df={df}')
    df.to_csv('Edita_nn_best_pso_ga.csv',header=False,mode='a')
