...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('CA1_20reg_1dv_18m_3par_cf.csv',header=False,mode='a')
    log.info(f'Cost function done: cost={cost}. ({FinalTime})')
    return cost

lowb=np.array([-18,-18,-18])
upbga=np.array([1,1,1])
...
plt.savefig('CA1_20reg_1dv_18m_3par_PSO.png')
plt.savefig('CA1_20reg_1dv_18m_3par_PSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('CA1_20reg_1dv_18m_3par_best_pso.csv')
...
for cnt in range(200):
    ...
    #plt.show()
    plt.savefig('CA1_20reg_1dv_18m_3par_GA.png')
    plt.savefig('CA1_20reg_1dv_18m_3par_GA.pdf')
    chrom = ga.Chrom
    ...
    if bpar_dist > 1e-7:
        ...
        df.index = [cfCounter]
        #log.info(f'best df={df}')
        df.to_csv('CA1_20reg_1dv_18m_3par_best_pso_ga.csv', header=False, mode='a')
    else :
        bestX = ga.best_x
        bestY = best_y
