...
def costFunction(par):
    ...
    if dumpCSV:
        ...
        df.to_csv('CA1_20reg_1dv_3w_cf.csv',header=False,mode='a')
...
plt.savefig('CA1_20reg_1dv_3w_PSO.png')
plt.savefig('CA1_20reg_1dv_3w_PSO.pdf')
...
psoDF['Cost']=psoy
psoDF.to_csv('CA1_20reg_1dv_3w_best_pso.csv')
...
for cnt in range(200):
    ...
    #plt.show()
    plt.savefig('CA1_20reg_1dv_3w_GA.png')
    plt.savefig('CA1_20reg_1dv_3w_GA.pdf')
    chrom = ga.Chrom
    ...
    if bpar_dist > 1e-7:
        ...
        df.index = [cfCounter]
        #log.info(f'best df={df}')
        df.to_csv('CA1_20reg_1dv_3w_best_pso_ga.csv', header=False, mode='a')
    else :
        bestX = ga.best_x
        bestY = best_y
